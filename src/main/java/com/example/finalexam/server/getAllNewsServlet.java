...
public class getAllNewsServlet extends HttpServlet {
	...
	String sql="select T_News.*, t_users.image as pictuser from T_News join t_users on t_users.email=T_News.creator order by T_News.id desc";
		...
		try {
			while(rs.next()) {
				...
				news.setCreatorImage(rs.getString("pictuser"));
//				news.setImage("");
				list.add(news);
			}
